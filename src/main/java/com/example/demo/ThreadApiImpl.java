...
public class ThreadApiImpl implements ThreadApi {
    ...
    private static final String SEARCH_USER_ID_BY_NICKNAME = "SELECT ID FROM USERS WHERE lower(nickname) = ?;";
    ...
    @ApiParam(value = "Список создаваемых постов." ,required=true ) @RequestBody Posts posts) {
        ...
        for (int i = 0; i < posts.size(); ++i) {
            ...
            BigDecimal postId = jdbcTemplate.queryForObject(CREATE_POST_QUERY, BigDecimal.class, post.getParent(), author, post.getThread(), forumResult.getId(), post.getMessage(), new Timestamp(new DateTime(post.getCreated()).getMillis()));
            post.setId(postId);
            PostPathBranch parentPathBranch = new PostPathBranch();
            String path = "";
            if (parent != null) {
                try {
                    parentPathBranch = jdbcTemplate.queryForObject(SEARCH_POST_PATH_AND_BRANCH_BY_POST_ID, new Object[]{ parent.getId() }, new PostPathBranchMapper());
                } catch (Exception e1) {}
            } else {
                try {
    ...
    public ResponseEntity<?> threadVote(@ApiParam(value = "Идентификатор ветки обсуждения.",required=true ) @PathVariable("slugOrId") String slugOrId,
        ...
        Vote old_vote = null;
        try {
            old_vote = jdbcTemplate.queryForObject(SEARCH_VOTE_BY_THREAD_ID_AND_USER_ID, new Object[] {thread.getId(), user_id}, new VoteMapper());
