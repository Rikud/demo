...
CREATE INDEX IF NOT EXISTS branchs_post_index on BRANCHS(post);
...
BEGIN
  ...
  IF parent_id ISNULL THEN
    SELECT MAX(branch) FROM BRANCHS INTO branch_id;
    INSERT INTO BRANCHS(branch, post) values(branch_id + 1, post_id);
  ELSE
